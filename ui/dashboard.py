...
def display_detailed_view(ticker):
    try:
        ...
        with col1:
            ...
            st.metric("Stop-Loss", f"{last_row['Stop_Loss']:.2f}", 
                      f"-{last_row['Close'] - last_row['Stop_Loss']:.2f} (Risk)", delta_color="inverse")
            st.metric("Take-Profit", f"{last_row['Take_Profit']:.2f}",
                      f"+{last_row['Take_Profit'] - last_row['Close']:.2f} (Reward)", delta_color="normal")
...
def run_app():
    ...
    with tabs[2]:
        ...
        if st.session_state.portfolio:
            ...
            if portfolio_data:
                total_pl, total_pl_pct = total_value - total_investment, (total_pl / total_investment - 1) * 100 if total_investment != 0 else 0
                c1, c2, c3 = st.columns(3)
